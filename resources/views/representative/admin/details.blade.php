                                    ...
                                    @foreach ($doctors as $doctor)
                                        <div class="bg-white border border-pink-200 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 p-5 hover:bg-pink-50">
                                            <div class="flex items-center justify-between gap-4">
                                                ...
                                                <div class="flex items-center space-x-2">
                                                    <a href="{{ route('doctors.edit', ['id' => $doctor->id]) }}" class="text-pink-600 hover:text-pink-800 transition duration-200 bg-pink-50 hover:bg-pink-100 p-2 rounded-full" title="Edit Doctor">
                                                        <i class="fi fi-sr-edit text-lg"></i>
                                                    </a>
                                                    <form action="{{ route('representatives.doctor.destroy', $doctor->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this doctor?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-600 hover:text-red-800 transition duration-200 bg-red-50 hover:bg-red-100 p-2 rounded-full" title="Delete Doctor">
                                            ...
                                            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                                                <div class="bg-pink-50 p-3 rounded-lg">
                                                    <p class="flex items-center"><i class="fi fi-sr-envelope mr-2 text-pink-600"></i> Email: {{ $doctor->email ?? 'N/A' }}</p>
                                                </div>
                                                <div class="bg-pink-50 p-3 rounded-lg">
                                                    <p class="flex items-center"><i class="fi fi-brands-whatsapp mr-2 text-pink-600"></i> Phone: {{ $doctor->phone ?? 'N/A' }}</p>
                                                </div>
                                                <div class="bg-pink-50 p-3 rounded-lg">
                                                    <p class="flex items-center"><i class="fi fi-sr-marker mr-2 text-pink-600"></i> Clinic Location: {{ $doctor->clinic_location ?? 'N/A' }}</p>
                                                </div>
                                                <div class="bg-pink-50 p-3 rounded-lg">
                                                    <p class="flex items-center"><i class="fi fi-sr-flag mr-2 text-pink-600"></i> Maunal Address: {{ $doctor->feedback ?? 'N/A' }}</p>
                                                </div>
                                            </div>
                                            <!-- Doctor Linked Location -->
                                            @if (isset($doctorLocations[$doctor->id]))
                                                @php $location = $doctorLocations[$doctor->id]; @endphp
                                                <div class="mt-4 bg-pink-50 p-3 rounded-lg">
                                                    <p class="text-sm font-semibold text-gray-700 mb-2">Linked Location</p>
                                                    <div class="flex flex-col sm:flex-row sm:items-center gap-2">
                                                        <p class="flex items-center text-sm">
                                                            <i class="fi fi-sr-map-marker mr-2 text-pink-600"></i>
                                                            <span class="location-address" data-lat="{{ $location->latitude }}" data-lng="{{ $location->longitude }}">Loading...</span>
                                                        </p>
                                                        <p class="flex items-center text-sm">
                                                            <i class="fi fi-sr-marker mr-2 text-pink-600"></i>
                                                        ...
                                                        <button class="show-map-btn bg-pink-600 text-white px-3 py-1 rounded-md hover:bg-pink-700 transition duration-200 shadow-sm text-sm" 
                                                                ...
                                                                data-type="Doctor Submission ({{ $doctor->name }})" 
                                                                data-date="{{ $location->created_at->format('Y-m-d H:i:s') }}"
                                                                data-address-id="address-doctor-{{ $doctor->id }}">
                                    ...
                                    <table class="min-w-full divide-y divide-pink-200">
                                        <thead class="bg-pink-50">
                                            <tr>
                                                ...
                                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Linked Location</th>
                                                ...
                                                <tr class="hover:bg-pink-50 transition-colors duration-200">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        @if (isset($doctorLocations[$doctor->id]))
                                                            ...
                                                            <div class="flex flex-col gap-1">
                                                                <span class="location-address" data-lat="{{ $location->latitude }}" data-lng="{{ $location->longitude }}">Loading...</span>
                                                                <span>{{ number_format($location->latitude, 7) }}, {{ number_format($location->longitude, 7) }}</span>
                                                                <button class="show-map-btn bg-pink-600 text-white px-3 py-1 rounded-md hover:bg-pink-700 transition duration-200 shadow-sm text-sm" 
                                                                        data-lat="{{ $location->latitude }}" 
                                                                        data-lng="{{ $location->longitude }}" 
                                                                        data-type="Doctor Submission ({{ $doctor->name }})" 
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        <div class="flex space-x-2">
                                                            <a href="{{ route('doctors.edit', ['id' => $doctor->id]) }}" class="text-pink-600 hover:text-pink-800 transition duration-200">
                                    ...
                                    @foreach ($locations as $location)
                                        <tr class="hover:bg-pink-50 transition-colors duration-200">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                <span class="px-2 py-1 text-xs rounded-full {{ in_array($location->type, ['In', 'Check-in']) ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
                                                    {{ $location->type }}
                                                </span>
                                            </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            showMapButtons.forEach(button => {
                ...
                const lat = parseFloat(button.dataset.lat);
                const lng = parseFloat(button.dataset.lng);
                const addressElement = button.closest('tr')?.querySelector('.location-address') || button.closest('.bg-pink-50')?.querySelector('.location-address');
                if (addressElement) {
